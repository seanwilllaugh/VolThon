...
func readColors() -> [hexColor]{
    let hexColors: [hexColor] = try! JSONDecoder().decode([hexColor].self, from: jsonData)
    print(hexColors.count)
    
    return hexColors
}

func findHex(color: String, hexColors: [hexColor]) -> String {
    
    for hexcolor in hexColors{
        if(hexcolor.name == color)
        {
            return hexcolor.hex
        }
    }
